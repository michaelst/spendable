...
function Budget() {
  ...
  const toggleBudgetArchived = (archivedAt: DateTime | null) => {
    updateBudget({
      variables: {
        id: id,
        input: { archivedAt }
  ...
  return (
    <>
      <div className="flex flex-col items-center py-16">
        {data.budget.name !== "Spendable" && (
          <div className="mb-2 bg-white ml-auto rounded-full p-2 px-3 mr-[15%]">
            <button className="mr-3" onClick={() => setShowForm(true)}>
              <FontAwesomeIcon icon={faPencil} />
            </button>
            {data.budget.archivedAt ? (
              <button onClick={() => toggleBudgetArchived(null)}>
                <FontAwesomeIcon icon={faBoxOpen} />
              </button>
            ) : (
              <button className="" onClick={() => toggleBudgetArchived(DateTime.now())}>
      ...
      <BudgetForm id={id} show={showForm} setShow={setShowForm} />
    </>
  )
}

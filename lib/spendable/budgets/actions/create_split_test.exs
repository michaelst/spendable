defmodule Spendable.Budgets.Actions.CreateSplitTest do
  ...
  test "creates a split with its lines", %{scope: scope, budget: budget} do
    assert {:ok, %Split{id: "spl_" <> _uxid, name: "Paycheck"} = split} =
    ...
    assert [%{id: "spll_" <> _line_uxid, amount: amount}] =
             split.split_lines

    assert Decimal.eq?(amount, "10.00")
  end

  test "gives the lines the split's owner", %{scope: scope, budget: budget} do
    ...
    assert [%{user_id: ^user_id}] = split.split_lines
  end
  ...
  test "rejects a line pointing at another user's budget", %{scope: scope} do
    ...
    assert %{split_lines: [%{budget_id: ["does not exist"]}]} =
             errors_on(changeset)
  end

  test "errors without a name", %{scope: scope} do
    assert {:error, changeset} = Budgets.create_split(scope, %{})
  ...
  test "errors when a line has no budget", %{scope: scope} do
    assert {:error, changeset} =
             Budgets.create_split(scope, %{
               "name" => "Paycheck",
               "split_lines" => %{"0" => %{"amount" => "10.00"}}
             })

    assert %{split_lines: [%{budget_id: ["can't be blank"]}]} =
             errors_on(changeset)
  end
end
